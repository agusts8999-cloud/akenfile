...
class ControlCenterController extends Controller
{
    ...
    public function index(): View
    {
        ...
        if (! isset($settings['preview_dialog_width_px'])) {
            $this->controlCenterService->setSetting('preview_dialog_width_px', '500', 'integer');
            $settings['preview_dialog_width_px'] = '500';
        }
        if (! isset($settings['file_thumbnail_size_px'])) {
            $this->controlCenterService->setSetting('file_thumbnail_size_px', '48', 'integer');
            $settings['file_thumbnail_size_px'] = '48';
        }
        ...
        $systemInfo = [
            ...
            'rows_per_page' => (int) ($settings['rows_per_page'] ?? 24),
            'preview_dialog_width_px' => (int) ($settings['preview_dialog_width_px'] ?? 500),
            'file_thumbnail_size_px' => (int) ($settings['file_thumbnail_size_px'] ?? 48),
    ...
    public function updateSettings(UpdateControlCenterRequest $request): RedirectResponse
    {
        ...
        $this->controlCenterService->setSetting('rows_per_page', (string) $request->input('rows_per_page'), 'integer');
        $this->controlCenterService->setSetting('preview_dialog_width_px', (string) $request->input('preview_dialog_width_px'), 'integer');
        $this->controlCenterService->setSetting('file_thumbnail_size_px', (string) $request->input('file_thumbnail_size_px'), 'integer');
        ...
        $controlCenterPassword = (string) ($request->input('control_center_password') ?? '');
        $controlCenterPasswordConfirmation = (string) ($request->input('control_center_password_confirmation') ?? '');
        if ($controlCenterPassword !== '' && $controlCenterPassword === $controlCenterPasswordConfirmation) {
            $this->controlCenterService->setSetting(
                'control_center_password',
                Hash::make($controlCenterPassword)
            );
        }

        $smtpPassword = (string) ($request->input('smtp_password') ?? '');
        $smtpPasswordConfirmation = (string) ($request->input('smtp_password_confirmation') ?? '');
        if ($smtpPassword !== '' && $smtpPassword === $smtpPasswordConfirmation) {
            $this->controlCenterService->setSetting(
                'smtp_password',
